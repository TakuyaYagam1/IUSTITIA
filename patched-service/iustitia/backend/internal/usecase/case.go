...
// PATCH - Vuln 7 (SQL injection in ORDER BY via squirrel raw concat).
// Было: req.OrderBy / req.Direction шли в SQLStore.SearchCases и там
// конкатенировались сырым образом в squirrel.OrderBy(...). Атакующий
// передавал что-то вроде
//
//	"0, (SELECT secret_payload FROM mtb_directives WHERE classification='top-secret' LIMIT 1)"
//
// и вытягивал SECRET_MARKER_Q.
// Стало: whitelist'им OrderBy и Direction В USECASE слое, ДО обращения к
// store. Любое значение вне whitelist молча сбрасывается в безопасный
// дефолт - сырые строки до SQL уже не доходят.
var (
...
func (u *Case) Dismiss(ctx context.Context, caseID uuid.UUID, _ string) error {
	n, err := u.store.DismissCase(ctx, caseID.String())
	if err != nil {
