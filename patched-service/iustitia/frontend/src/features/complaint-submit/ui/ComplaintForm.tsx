...
export const ComplaintForm = ({ onSuccess }: ComplaintFormProps): JSX.Element => {
  ...
  // Селектор дел на форме доноса использует /api/cases/search, потому что
  // /api/cases закрыт для citizen бекендом (см. router.go: RequireRole(Judge,
  // Prosecutor)). /search открыт для всех авторизованных ролей.
  ...
  const cases = useMemo(
    () => (casesQuery.data ?? []).filter((c) => c.status === 'open' || c.status === 'hearing'),
    [casesQuery.data],
  );

  // UX: при выборе дела автоподставляем фигуранта из картотеки - citizen не
  // переписывает «Фамилия, Имя, Купол №N» вручную. Статью оставляем пустой:
  // квалификация - дело доносителя.
  useEffect(() => {
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div className={styles.actions}>
        <span className={styles.hint}>Каждое заявление фиксируется в картотеке.</span>
        <Button type="submit" variant="primary" disabled={isSubmitting || mutation.isPending}>
          {mutation.isPending ? 'Передача...' : 'Подать заявление'}
        </Button>
      </div>
    </form>
  );
