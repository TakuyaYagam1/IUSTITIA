...
const JUDGE_FILTERS: { value: StatusFilter; label: string }[] = [
  { value: 'all', label: 'Все' },
  { value: 'hearing', label: 'Слушание' },
  { value: 'closed', label: 'Закрытые' },
];

const ALL_FILTERS: { value: StatusFilter; label: string }[] = [
  { value: 'all', label: 'Все' },
...
export const CasesPage = (): JSX.Element => {
  ...
  const STATUS_FILTERS = isJudge ? JUDGE_FILTERS : ALL_FILTERS;

  const filtered = useMemo(() => {
    if (!data) return [];
    if (isProsecutor) {
      return data.filter((c) => c.status === 'assigned' && c.assigned_prosecutor_id === userId);
    }
    if (isJudge) {
      const judgeBase = data.filter((c) => c.status === 'hearing' || c.status === 'closed');
      if (status === 'all') return judgeBase;
      return judgeBase.filter((c) => c.status === status);
    }
    if (status === 'all') return data;
    return data.filter((c) => c.status === status);
  }, [data, status, isProsecutor, isJudge, userId]);
